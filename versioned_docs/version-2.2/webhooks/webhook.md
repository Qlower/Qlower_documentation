...
| **400-499** | Erreur client (ex: format invalide) | Marque comme échouée |
...
ComptAppart **ne retente pas automatiquement** l'envoi d'une notification en échec : elle reste marquée `failed` définitivement, quel que soit le code d'erreur ou un timeout. Si vous constatez un webhook manquant, contactez-nous (voir [Support](./errors#support)) avec l'`event_id` ou l'`order_id` concerné — nous investiguerons et renverrons la notification manuellement si nécessaire.
:::

Votre endpoint doit donc répondre de façon fiable dès le premier envoi. Si votre traitement peut échouer ponctuellement (timeout, dépendance externe indisponible…), traitez la notification de façon asynchrone (voir [Timeout](./errors#timeout--30s)) plutôt que de compter sur un renvoi de notre part.

---

## Idempotence

Une notification peut malgré tout vous parvenir en double dans de rares cas (par exemple un renvoi manuel après incident). Vous devez gérer l'idempotence côté serveur :
